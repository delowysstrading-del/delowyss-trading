# main.py - V5.4 PREMIUM COMPLETA CORREGIDA PARA RENDER
"""
    ...
    <body>
        <div class="container">
            ...
            <div class="prediction-card">
                <h2 id="prediction-direction">{direction} — {confidence}% de confianza</h2>
                <p>Precio actual: <strong>{current_price:.5f}</strong></p>
                <p>Ticks procesados: <strong>{tick_count}</strong></p>
                <div id="reasons-list">
                    {"".join([f"<p>✅ {r}</p>" for r in current_prediction.get('reasons', [])])}
                </div>
            </div>
            ...
            <div class="metrics-grid">
                <div class="metric-cell">
                    ...
                    <div id="system-status">ACTIVO</div>
                </div>
                <div class="metric-cell">
                    <strong>Progreso Vela</strong>
                    <div id="candle-progress">{(current_prediction.get('candle_progress', 0)*100):.1f}%</div>
                </div>
                <div class="metric-cell">
                    <strong>Fase Mercado</strong>
                    <div id="market-phase">{current_prediction.get('market_phase', 'N/A')}</div>
                </div>
            </div>
        </div>
                ...
                fetch('/api/prediction')
                    ...
                    .then(data => {{
                        document.getElementById('prediction-direction').textContent = 
                            `${{data.direction}} — ${{data.confidence}}% de confianza`;
                        document.getElementById('candle-progress').textContent = 
                            `${{(data.candle_progress * 100).toFixed(1)}}%`;
                        document.getElementById('market-phase').textContent = data.market_phase;
                    }})
...
# ✅ CORRECCIÓN: Solo iniciar el sistema si es el archivo principal
if __name__ == "__main__":
    ...
    # Para Render/Gunicorn - iniciar el sistema automáticamente
    start_system()
