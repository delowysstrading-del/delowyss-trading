...
def generate_html_interface():
    """Interfaz HTML completa y funcional generada automáticamente (preserva estilo original)."""
    direction = current_prediction.get("direction", "N/A")
    confidence = current_prediction.get("confidence", 0)
    current_price = current_prediction.get("current_price", 0.0)
    tick_count = current_prediction.get("tick_count", 0)
    market_phase = current_prediction.get("market_phase", "N/A")
    candle_progress = round(current_prediction.get("candle_progress", 0) * 100, 2)
    reasons = current_prediction.get("reasons", []) or []
    timestamp = current_prediction.get("timestamp", now_iso())
...
<title>Delowyss AI Premium V5.4</title>
<style>
  :root{{--bg:#0d1117;--card:#161b22;--muted:#cbd5e1;--accent:#48bb78;--danger:#f56565;--soft:#94a3b8}}
  body{{margin:0;font-family:Inter,Segoe UI,system-ui,-apple-system,Roboto,'Helvetica Neue',Arial;background:var(--bg);color:var(--muted)}}
  ...
  .metric{font-size:1.05rem;margin:8px 0}
  .big{font-size:1.6rem;font-weight:700;color:var(--muted)}
  .small{font-size:0.9rem;color:var(--soft)}
  .badge{display:inline-block;padding:6px 10px;border-radius:999px;background:rgba(72,187,120,0.12);color:var(--accent);font-weight:600}
  pre{{white-space:pre-wrap;word-break:break-word;font-size:0.85rem;color:var(--soft)}}
  footer{{text-align:center;padding:12px;color:var(--soft)}}
  .reasons{max-height:120px;overflow:auto;padding:8px;background:rgba(255,255,255,0.02);border-radius:6px}
</style>
<script>
async function updateData() {{
  try {{
    ...
    document.getElementById('direction').innerText = json.direction;
    document.getElementById('confidence').innerText = json.confidence + '%';
    document.getElementById('price').innerText = Number(json.current_price).toFixed(5);
    document.getElementById('ticks').innerText = json.tick_count;
    document.getElementById('phase').innerText = json.market_phase || 'N/A';
    document.getElementById('progress').innerText = (json.candle_progress*100).toFixed(1) + '%';
    document.getElementById('timestamp').innerText = json.timestamp || new Date().toISOString();

    // Reasons list
    const reasons = json.reasons || [];
    const rbox = document.getElementById('reasons');
    rbox.innerHTML = '';
    if (reasons.length===0) {{ rbox.innerHTML = '<div class="small">Sin razones registradas.</div>'; }}
    reasons.forEach(r=>{{ const d = document.createElement('div'); d.className='small'; d.innerText = '• ' + r; rbox.appendChild(d); }});

    // Color hints
    const dir = json.direction;
    const dirEl = document.getElementById('direction');
    dirEl.className = dir==='ALZA' ? 'big' : dir==='BAJA' ? 'big' : 'big';
...
<body>
<header><h1>Delowyss Trading AI — V5.4 PREMIUM</h1></header>
<main class="wrap">
  <div class="grid">
    <div class="card">
      <div class="metric small">Dirección</div>
      <div id="direction" class="big">{direction}</div>
      <div class="metric small">Confianza <span class="badge" id="confidence">{confidence}%</span></div>
      <div class="metric small">Precio actual <span class="big" id="price">{current_price:.5f}</span></div>
    </div>
    <div class="card">
      <div class="metric small">Ticks procesados</div>
      <div id="ticks" class="big">{tick_count}</div>
      <div class="metric small">Progreso vela</div>
      <div id="progress" class="big">{candle_progress}%</div>
    </div>
    <div class="card">
      <div class="metric small">Fase de mercado</div>
      <div id="phase" class="big">{market_phase}</div>
      <div class="metric small">Última actualización</div>
      <div id="timestamp" class="small">{timestamp}</div>
    </div>
    <div class="card">
      <div class="metric small">Razones del sistema</div>
      <div id="reasons" class="reasons"></div>
    </div>
  </div>

  <div style="margin-top:16px" class="card">
    <div class="metric small">Accesos rápidos API</div>
    <pre>/api/prediction  - Estado actual JSON
/api/validation  - Última validación y rendimiento
/api/health      - Estado de salud del sistema
/api/system-info - Información del sistema y conexión
</pre>
  </div>
</main>
<footer>© {datetime.utcnow().year} Eduardo Solis — Delowyss AI</footer>
</body>
</html>\"\"\"
    return html
